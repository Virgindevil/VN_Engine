...
namespace VNKit
{
    ...
    public class VisualNovelEngine : MonoBehaviour
    {
        ...
        /// <summary>
        /// Strip a language suffix from a script asset name so "Demo.ru", "Demo_RU"
        /// and "01_journey_start_RUS" all resolve to the same base id.
        /// </summary>
        public static string CanonicalScriptName(string name)
        {
            if (string.IsNullOrEmpty(name)) return name;
            int cut = LanguageSuffixStart(name);
            return cut > 0 ? name.Substring(0, cut) : name;
        }

        static int LanguageSuffixStart(string name)
        {
            int dot = name.LastIndexOf('.');
            if (dot > 0 && IsLanguageSuffix(name.Substring(dot + 1))) return dot;
            int us = name.LastIndexOf('_');
            if (us > 0 && IsLanguageSuffix(name.Substring(us + 1))) return us;
            return -1;
        }

        /// <summary>True for 2/3-letter language tags: ru, rus, en, eng, ja, ...</summary>
        public static bool IsLanguageSuffix(string suffix)
        {
            ...
            switch (s)
            {
                ...
                case "ru": case "rus":
                case "ja": case "jpn":
                case "zh": case "zho": case "chi":
                case "de": case "deu": case "ger":
                case "fr": case "fra": case "fre":
                case "es": case "spa":
                case "ko": case "kor":
                    return true;
            }
        ...
        /// <summary>
        /// Returns a script by name. The name is first reduced to a language-less
        /// base ("Demo.ru" / "Demo_RU" / "Demo" → "Demo"), then a variant matching
        /// Settings.language is chosen: Name.&lt;lang&gt; (Demo.ru), Name_&lt;lang&gt; /
        /// Name_&lt;LANG&gt; / Name_RUS, then the base file. Parsed scripts keep the
        /// canonical name so saves, goto and language switches stay aligned.
        /// </summary>
        public VNScript GetScript(string name)
        {
            if (string.IsNullOrEmpty(name)) return null;

            string canon = CanonicalScriptName(name);
            string resolved = ResolveScriptAssetName(canon);
            if (resolved == null)
            {
            ...
            TextAsset asset;
            if (!scriptAssets.TryGetValue(resolved, out asset) || asset == null)
            {
                VNLog.Warn("Script not found: '" + name + "'. Add it to the engine's script list.");
                return null;
            }
            s = VNScriptParser.Parse(canon, asset.text);
            scriptCache[resolved] = s;
            return s;
        }

        /// <summary>Pick the registered TextAsset key for a canonical script name.</summary>
        string ResolveScriptAssetName(string canon)
        {
            string lang = Settings != null ? Settings.language : null;
            if (!string.IsNullOrEmpty(lang))
            {
                string[] preferred =
                {
                    canon + "." + lang,
                    canon + "_" + lang,
                    canon + "_" + lang.ToUpperInvariant()
                };
                for (int i = 0; i < preferred.Length; i++)
                    if (scriptAssets.ContainsKey(preferred[i])) return preferred[i];

                string langLower = lang.ToLowerInvariant();
                string prefixDot = canon + ".";
                string prefixUs = canon + "_";
                foreach (var key in scriptAssets.Keys)
                {
                    if (key.Length <= canon.Length) continue;
                    string rest = null;
                    if (key.StartsWith(prefixDot, StringComparison.Ordinal)) rest = key.Substring(prefixDot.Length);
                    else if (key.StartsWith(prefixUs, StringComparison.Ordinal)) rest = key.Substring(prefixUs.Length);
                    if (rest == null || !IsLanguageSuffix(rest)) continue;
                    if (rest.ToLowerInvariant().StartsWith(langLower, StringComparison.Ordinal))
                        return key;
                }
            }

            if (scriptAssets.ContainsKey(canon)) return canon;

            // Only a localized file is registered (e.g. just 01_journey_start_RUS).
            foreach (var key in scriptAssets.Keys)
                if (CanonicalScriptName(key) == canon)
                    return key;
            return null;
        }
        ...
        /// <summary>Rewrite the last backlog line after a live language switch.</summary>
        public void ReplaceLastBacklog(string speaker, string text)
        {
            if (backlog.Count == 0) return;
            var e = backlog[backlog.Count - 1];
            e.speaker = speaker;
            e.text = text;
        }
        ...
        void RelocalizeUI()
        {
            ...
            // Swap the running .vns to the variant of the new language and
            // refresh the line / choice currently on screen.
            if (Player != null) Player.RelocalizeScript();
        }
